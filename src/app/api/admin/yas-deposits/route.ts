  ...
  try {
    ...
    await db.$transaction([
      db.yasDeposit.update({
        where: { id: depositId },
        data: { status: 'approved', adminNote: adminNote || 'Conversion effectuée. TRX envoyé à votre adresse.' },
      }),
      db.user.update({
      ...
      db.transaction.create({
        data: {
          ...
          detail: `Yas du Togo conversion approved: $${deposit.amountUsd.toFixed(2)} → ${deposit.amountTrx.toFixed(2)} TRX credited`,
          userId: deposit.userId,
        },
      }),
    ]);

    return NextResponse.json({ success: true, message: 'Conversion approuvée et solde crédité' });
  } catch (error) {
    return NextResponse.json({ success: false, error: String(error) }, { status: 500 });
  }
}
