...
function SplashScreen({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div className={`absolute inset-0 bg-[#F8F9FA] z-[9999] flex flex-col items-center justify-center transition-all duration-500 ${hide ? 'opacity-0 invisible' : ''}`}>
      ...
      <div className="absolute w-[160px] h-[160px] rounded-full bg-[rgba(34,197,94,0.06)] blur-[80px] bottom-[20%] right-[10%]" style={{ animation: 'orbFloat 6s ease-in-out infinite 3s reverse' }} />
      <LogoImg className="w-[120px] h-[120px] mb-9" style={{ animation: 'spFloat 3s ease-in-out infinite', filter: 'drop-shadow(0 0 40px rgba(34,197,94,0.2))' }} />
  ...
  return (
    <section className="absolute inset-0 bg-[#F8F9FA] flex flex-col items-center justify-center z-[200]">
      <div className="w-full max-w-[330px] text-center px-5">
        <LogoImg className="w-[100px] h-[100px] mx-auto mb-4" style={{ filter: 'drop-shadow(0 4px 20px rgba(34,197,94,0.2))' }} />
        <h1 className="text-[1.8rem] font-black mb-1 text-[#22C55E] tracking-[2px]">BE RICH</h1>
        ...
        {mode === 'login' ? (
          <form onSubmit={handleLogin}>
            <div className="mb-4 w-full"><label className="block mb-1.5 text-[0.75rem] font-semibold text-[rgba(0,0,0,0.45)]">Email</label><input name="email" type="email" required placeholder="votre@email.com" className="w-full py-3 px-4 bg-[#F3F4F6] border-[1.5px] border-[rgba(0,0,0,0.08)] rounded-xl text-[0.88rem] outline-none text-gray-900 placeholder:text-[rgba(0,0,0,0.3)] focus:border-[#22C55E]" /></div>
            <div className="mb-4 w-full relative"><label className="block mb-1.5 text-[0.75rem] font-semibold text-[rgba(0,0,0,0.45)]">Mot de passe</label><input name="password" type={showPw.l ? 'text' : 'password'} required placeholder="••••••••" className="w-full py-3 px-4 pr-11 bg-[#F3F4F6] border-[1.5px] border-[rgba(0,0,0,0.08)] rounded-xl text-[0.88rem] outline-none text-gray-900 placeholder:text-[rgba(0,0,0,0.3)] focus:border-[#22C55E]" /><button type="button" onClick={() => setShowPw({ ...showPw, l: !showPw.l })} className="absolute right-3 top-[38px] bg-transparent border-none text-[rgba(0,0,0,0.35)] cursor-pointer p-0.5"><i className={`fas ${showPw.l ? 'fa-eye-slash' : 'fa-eye'}`}></i></button></div>
            <button type="submit" disabled={loading} className="w-full py-3.5 rounded-xl bg-[#22C55E] text-[#050506] font-bold text-[0.88rem] border-none cursor-pointer shadow-[0_4px_20px_rgba(34,197,94,0.2)] transition-transform active:scale-[0.97] disabled:opacity-60 flex items-center justify-center gap-2">{loading ? <div className="w-4 h-4 border-2 border-[rgba(5,5,6,0.3)] border-t-[#050506] rounded-full" style={{ animation: 'spin 0.6s linear infinite' }} /> : <><i className="fas fa-arrow-right"></i> Se connecter</>}</button>
          </form>
        ) : (
          <form onSubmit={handleRegister}>
            <div className="mb-2.5 w-full"><label className="block mb-1 text-[0.72rem] font-semibold text-[rgba(0,0,0,0.45)]">Nom complet</label><input name="name" type="text" required placeholder="Jean Dupont" minLength={2} className={`w-full py-2.5 px-3.5 bg-[#F3F4F6] border-[1.5px] ${errors.name ? 'border-[#F87171]' : 'border-[rgba(0,0,0,0.08)]'} rounded-xl text-[0.85rem] outline-none text-gray-900 placeholder:text-[rgba(0,0,0,0.3)] focus:border-[#22C55E]`} />{errors.name && <p className="text-[#F87171] text-[0.65rem] mt-0.5">{errors.name}</p>}</div>
            <div className="mb-2.5 w-full"><label className="block mb-1 text-[0.72rem] font-semibold text-[rgba(0,0,0,0.45)]">Email</label><input name="email" type="email" required placeholder="votre@email.com" className="w-full py-2.5 px-3.5 bg-[#F3F4F6] border-[1.5px] border-[rgba(0,0,0,0.08)] rounded-xl text-[0.85rem] outline-none text-gray-900 placeholder:text-[rgba(0,0,0,0.3)] focus:border-[#22C55E]" /></div>
            <div className="mb-2.5 w-full"><label className="block mb-1 text-[0.72rem] font-semibold text-[rgba(0,0,0,0.45)]">Mot de passe</label><input name="password" type={showPw.r ? 'text' : 'password'} required placeholder="Min. 6 caractères" minLength={6} className={`w-full py-2.5 px-3.5 bg-[#F3F4F6] border-[1.5px] ${errors.password ? 'border-[#F87171]' : 'border-[rgba(0,0,0,0.08)]'} rounded-xl text-[0.85rem] outline-none text-gray-900 placeholder:text-[rgba(0,0,0,0.3)] focus:border-[#22C55E]`} />{errors.password && <p className="text-[#F87171] text-[0.65rem] mt-0.5">{errors.password}</p>}</div>
            <div className="mb-2.5 w-full"><label className="block mb-1 text-[0.72rem] font-semibold text-[rgba(0,0,0,0.45)]">Confirmer</label><input name="password2" type={showPw.r2 ? 'text' : 'password'} required placeholder="••••••••" className={`w-full py-2.5 px-3.5 bg-[#F3F4F6] border-[1.5px] ${errors.password2 ? 'border-[#F87171]' : 'border-[rgba(0,0,0,0.08)]'} rounded-xl text-[0.85rem] outline-none text-gray-900 placeholder:text-[rgba(0,0,0,0.3)] focus:border-[#22C55E]`} />{errors.password2 && <p className="text-[#F87171] text-[0.65rem] mt-0.5">{errors.password2}</p>}</div>
            <div className="mb-2.5 w-full"><label className="block mb-1 text-[0.72rem] font-semibold text-[rgba(0,0,0,0.45)]">Code de parrainage <span className="opacity-50">(optionnel)</span></label><input name="referralCode" type="text" placeholder="BR-XXXXXX" className="w-full py-2.5 px-3.5 bg-[#F3F4F6] border-[1.5px] border-[rgba(0,0,0,0.08)] rounded-xl text-[0.85rem] outline-none text-gray-900 placeholder:text-[rgba(0,0,0,0.3)] focus:border-[#22C55E]" /></div>
    ...
    <>
      ...
      <div className="px-[18px] py-4 flex-1 w-full overflow-y-auto">
        ...
        <div className="bg-[#FFFFFF] border border-[rgba(0,0,0,0.08)] rounded-2xl p-5 mb-4 relative overflow-hidden">
          {/* Subtle gold glow */}
          <div className="absolute -top-20 -right-20 w-[200px] h-[200px] bg-[radial-gradient(circle,rgba(34,197,94,0.1),transparent_60%)]" style={{ animation: 'orbFloat 8s ease-in-out infinite' }} />
          <div className="absolute -bottom-10 -left-10 w-[140px] h-[140px] bg-[radial-gradient(circle,rgba(34,197,94,0.06),transparent_60%)]" style={{ animation: 'orbFloat 8s ease-in-out infinite 4s reverse' }} />
          <div className="relative z-[1]">
            <div className="flex items-center justify-between mb-1">
              ...
              {user.depositCount > 0 && (
                <span className="bg-[rgba(34,197,94,0.12)] text-[#22C55E] text-[0.6rem] font-bold px-2.5 py-[3px] rounded-full border border-[rgba(34,197,94,0.15)]">{user.depositCount} dépôt{user.depositCount > 1 ? 's' : ''}</span>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              ...
              <div className="bg-[#F3F4F6] rounded-lg p-2.5 border border-[rgba(0,0,0,0.08)] flex items-center gap-2">
                <div className="w-7 h-7 rounded-md bg-[rgba(245,158,11,0.12)] flex items-center justify-center shrink-0"><i className="fas fa-bolt text-[0.6rem] text-[#F59E0B]"></i></div>
                <div className="min-w-0">
              ...
              <div className="bg-[#F3F4F6] rounded-lg p-2.5 border border-[rgba(0,0,0,0.08)] flex items-center gap-2">
                <div className="w-7 h-7 rounded-md bg-[rgba(139,92,246,0.12)] flex items-center justify-center shrink-0"><i className="fas fa-building text-[0.6rem] text-[#8B5CF6]"></i></div>
                <div className="min-w-0">
        ...
        <div className="flex gap-2 mb-4">
          {[
            { icon: 'fa-wallet', label: 'Wallet', page: 'wallet', color: '#22C55E', bg: 'rgba(34,197,94,0.12)' },
            { icon: 'fa-arrow-down', label: 'Déposer', page: 'deposit', color: '#F59E0B', bg: 'rgba(245,158,11,0.12)' },
            { icon: 'fa-chart-line', label: 'Investir', page: 'finance', color: '#3B82F6', bg: 'rgba(59,130,246,0.12)' },
            { icon: 'fa-bolt', label: 'Trader', page: 'finance', color: '#F87171', bg: 'rgba(248,113,113,0.12)' },
            { icon: 'fa-building', label: 'Projets', page: 'finance', color: '#8B5CF6', bg: 'rgba(139,92,246,0.12)' },
          ].map((a, i) => (
            <button key={i} onClick={() => setPage(a.page)} className="flex-1 bg-[#FFFFFF] rounded-xl py-2.5 px-1 text-center border border-[rgba(0,0,0,0.08)] cursor-pointer transition-transform active:scale-95">
              <div className="w-9 h-9 rounded-xl mx-auto mb-1 flex items-center justify-center" style={{ backgroundColor: a.bg }}><i className={`fas ${a.icon} text-[0.8rem]`} style={{ color: a.color }}></i></div>
        ...
        {/* AI Tip Card */}
        <div className="bg-[#FFFFFF] border border-[rgba(139,92,246,0.15)] rounded-xl p-3.5 mb-4 flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[rgba(139,92,246,0.12)] flex items-center justify-center shrink-0 border border-[rgba(139,92,246,0.15)]"><i className="fas fa-robot text-[#8B5CF6] text-[0.9rem]"></i></div>
        ...
        <button onClick={() => setPage('guide')} className="w-full bg-[#FFFFFF] border border-[rgba(0,0,0,0.08)] rounded-xl p-3.5 mb-4 flex items-center gap-3 cursor-pointer transition-transform active:scale-[0.98]">
          <div className="w-10 h-10 rounded-xl bg-[rgba(20,184,166,0.12)] flex items-center justify-center shrink-0"><i className="fas fa-compass text-[#14B8A6] text-[1rem]"></i></div>
        ...
        <button onClick={() => setPage('profile')} className="w-full bg-[#FFFFFF] border border-[rgba(245,158,11,0.15)] rounded-xl p-3 mb-4 flex items-center gap-3 cursor-pointer transition-transform active:scale-[0.98]">
          <div className="w-9 h-9 rounded-xl bg-[rgba(245,158,11,0.12)] flex items-center justify-center shrink-0"><i className="fas fa-gift text-[#F59E0B] text-[0.9rem]"></i></div>
          <div className="flex-1 text-left">
...
function WalletScreen() {
  ...
  const accounts = [
    { key: 'principal', label: 'Compte Principal', balance: user.balance, icon: 'fa-wallet', iconColor: '#22C55E', iconBg: 'bg-[rgba(34,197,94,0.12)]' },
    ...
    { key: 'trade', label: 'Compte de Trading', balance: user.tradeBalance, icon: 'fa-bolt', iconColor: '#F59E0B', iconBg: 'bg-[rgba(245,158,11,0.12)]' },
    { key: 'project', label: 'Compte de Projet', balance: user.projectBalance, icon: 'fa-building', iconColor: '#8B5CF6', iconBg: 'bg-[rgba(139,92,246,0.12)]' },
  ] as const;

  return (
    <>
      <Header title="Portefeuille" icon="fa-wallet" iconColor="#22C55E" leftElement={<button onClick={() => setPage('home')} className="w-9 h-9 rounded-full flex items-center justify-center bg-[#F3F4F6] text-[rgba(0,0,0,0.55)] cursor-pointer border-none mr-1"><i className="fas fa-arrow-left text-[0.8rem]"></i></button>} rightElement={<button onClick={refresh} className="w-9 h-9 rounded-[10px] flex items-center justify-center bg-[#F3F4F6] text-[rgba(0,0,0,0.55)] cursor-pointer border-none"><i className={`fas fa-sync-alt text-[0.7rem] ${refreshing ? 'animate-spin' : ''}`} /></button>} />
      <div className="px-[18px] py-4 flex-1 w-full overflow-y-auto">
        ...
        <div className="bg-[#FFFFFF] text-gray-900 rounded-2xl p-5 mb-3 relative overflow-hidden border border-[rgba(0,0,0,0.08)] shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
          <div className="absolute -top-20 -right-20 w-[200px] h-[200px] bg-[radial-gradient(circle,rgba(34,197,94,0.1),transparent_60%)]" style={{ animation: 'orbFloat 8s ease-in-out infinite' }} />
          <div className="absolute -bottom-10 -left-10 w-[140px] h-[140px] bg-[radial-gradient(circle,rgba(34,197,94,0.08),transparent_60%)]" style={{ animation: 'orbFloat 8s ease-in-out infinite 4s reverse' }} />
          <div className="relative z-[1]">
            ...
            <div className="flex gap-2">
              <button onClick={() => setPage('deposit')} className="flex-1 py-[11px] rounded-xl text-[0.78rem] font-semibold cursor-pointer flex items-center justify-center gap-1.5 border-none bg-[#22C55E] text-[#050506] shadow-[0_2px_8px_rgba(34,197,94,0.15)]"><i className="fas fa-arrow-down"></i> Déposer</button>
          ...
          <div className="fixed inset-0 bg-[rgba(0,0,0,0.3)] backdrop-blur-md z-[6000] flex items-center justify-center" onClick={() => setTransferTarget(null)}>
            <div className="bg-[#FFFFFF] rounded-2xl w-[88%] max-w-[340px] border border-[rgba(0,0,0,0.08)] shadow-[0_8px_24px_rgba(0,0,0,0.1)] overflow-hidden" style={{ animation: 'modalIn 0.3s cubic-bezier(0.34,1.56,0.64,1)' }} onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-5">
                ...
                <input type="number" step="0.01" value={transferAmt} onChange={(e) => setTransferAmt(e.target.value)} placeholder="Montant (min 2 $)" className="w-full py-3.5 px-4 bg-[#F3F4F6] border-[1.5px] border-[rgba(0,0,0,0.08)] rounded-xl text-[0.88rem] outline-none mb-4 focus:border-[#22C55E] font-semibold text-[#1F2937]" />
                <div className="flex gap-2">
                  ...
                  <button onClick={handleTransfer} disabled={transferring} className="flex-1 py-3.5 rounded-xl bg-[#22C55E] text-[#050506] font-bold text-[0.82rem] border-none cursor-pointer disabled:opacity-60 transition-transform active:scale-95 shadow-[0_4px_16px_rgba(34,197,94,0.15)]">{transferring ? '...' : 'Confirmer'}</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {[
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-xl text-[0.75rem] font-semibold border-none cursor-pointer transition-all flex items-center justify-center gap-1.5 ${
              subTab === t.id
                ? 'bg-[#22C55E] text-[#050506] shadow-[0_2px_8px_rgba(34,197,94,0.15)]'
...
function BottomNav() {
  ...
  return (
    <nav className="h-[60px] bg-[#FFFFFF] border-t border-[rgba(0,0,0,0.08)] flex items-center justify-around px-2 shrink-0 safe-area-bottom">
      {tabs.map(t => (
        <button key={t.id} onClick={() => setPage(t.id)} className={`flex flex-col items-center justify-center py-1.5 px-2 border-none cursor-pointer transition-all ${isActive(t.id) ? 'text-[#22C55E]' : 'text-[rgba(0,0,0,0.35)]'}`}>
...
export default function BeRichApp() {
  ...
  if (!initialized) {
    return (<div className="bg-[#F8F9FA] h-screen flex items-center justify-center"><div className="w-8 h-8 border-[3px] border-[rgba(0,0,0,0.08)] border-t-[#22C55E] rounded-full" style={{ animation: 'spin 0.7s linear infinite' }} /></div>);
  }
